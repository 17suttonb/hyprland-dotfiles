...
readonly PACMAN_PACKAGES=(
    ...
    hyprpolkitagent             # Hyprland polkit daemon
    dconf                       # Key-value config system
    bluez                       # Bluetooth protocol stack
    bluez-utils                 # CLI tools for bluez
    bluez-tools
    
    # Build tools (rustup is included in the base tools)
    cpio                        # Low-level archive CLI utility
    cmake                       # Builds C programs
    ...
    flatpak         # Sandboxed application backend
    ...
    bc              # Converts constants into internal decimal numbers
    thefuck         # Corrects previous console command
    docker          # Container runtime/daemon
    docker-compose  # Docker plugin for multi-container docker apps
    strace          # Debug util
    powertop        # Intel tool for power-saving/energy monitoring
    man-db          # Manual pages
    ...
    hyprutils           # Shared implementations of commonly used types
    hyprgraphics        # Library for graphics related utilities, e.g. load image, color calcs
    hyprland-guiutils   # GUI Utils for some apps
    hyprlang            # Hyprland configuration parser
    libcanberra         # Sound event library for desktop notification sounds e.g. login, error
...
create_systemd_services() {
    ...
    # ALIFE: you can use this yourself if you want, it's just the service file has apple silicon driver specific stuff in it
...
main() {
    ...
    echo "Extra notes:"
    echo "Vesktop has been installed in place of Discord"
    echo "Chromium has been installed in place of Chrome"
    echo ""
    echo "You can swap these out for the original if you like, up to you"
    echo "Also I recommend doing a search all on the .config folder for \"wallpaper\" and fixing any incorrect references"
    echo "The plugin hyprexpo is super useful but it's commented out because apple silicon, you can run hyprpm enable hyprexpo if you would like to use it"
